...
if uploaded_file is not None:
    model = pickle.load(io.BytesIO(uploaded_file.read()))
    st.success("Model loaded successfully!")

    def run():
        # Load and display logo
        img1 = Image.open("SBI-Logo.png")  # Ensure this file is accessible
        ...
        if st.button("Submit"):
            ...
            if ans == 0:
                st.error(f"Hello: {fn} || Account Number: {account_no} || "
                         "According to our calculations, you will NOT get the loan from the 
